...
def webhook():
    ...
    if state["state"] == "search_ville_dest":
        ...
        if results:
            ...
            for r in results:
                note = r["moyenne"] if r["moyenne"] is not None else "Aucune note"
                dernier_avis = r["dernier_avis"] or "Aucun avis"
                response += (
                    f"• *{r['nom']}*\n"
                    f"📅 {r['date_depart']} – {r['ville_depart']} -> {r['ville_destination']}\n"
                    f"💬 {r['description']}\n"
                    f"⭐ Note : {note}\n"
                    f"📝 Dernier avis : {dernier_avis}\n"
